// src/components/admin/ProductManager.js - Optimizado para evitar pérdida de foco
...
const ProductManager = () => {
  ...
  const [brands] = useState([
    'Apple', 'Samsung', 'Google', 'Xiaomi', 'Sony', 'HP', 'Dell', 'Asus',
    'Nike', 'Adidas', 'IKEA', 'Centrum'
  ]);
  ...
  // Estados para formularios dinámicos - Separados para evitar re-renders
  const [newSpec, setNewSpec] = useState({ type: '', label: '', value: '' });
  ...
  // Memoizar productos filtrados para evitar re-cálculos
  ...
  const handleSaveProduct = useCallback(() => {
    ...
    const productData = {
      ...
      subcategoryName: getCategoryDisplayName(formData.category),
      ...
      inStock: formData.isActive && parseInt(formData.stockQuantity) > 0,
      discount: discount,
    ...
    setShowModal(false);
  }, [formData, modalMode, selectedProduct, addProduct, updateProduct, validateForm, getCategoryDisplayName]);

  // Función optimizada para manejar cambios en inputs
  ...
  // Funciones optimizadas para especificaciones
  const addSpecification = useCallback(() => {
    if (newSpec.type && newSpec.label && newSpec.value) {
      const selectedType = specificationTypes.find(t => t.type === newSpec.type);
      const spec = {
        type: newSpec.type,
        label: selectedType ? selectedType.label : newSpec.label,
        value: newSpec.value
      };
  ...
  // Funciones optimizadas para características
  ...
  // Funciones optimizadas para variantes
  ...
  // Funciones optimizadas para tags
  ...
  // Componente del modal optimizado
  const ProductModal = () => (
